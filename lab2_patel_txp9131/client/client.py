...
class myThread(threading.Thread):
    ...
    def Desynchronize(self):
        print("entered in Desynchronize")
        h = os.getcwd()
        h1 = h+"/"+self.oop
        print("directory needs be deleted",h1)
        ...
        hw3  = str(hw1[0]) + "/server"+"/track.pickle"
        with open(hw3,"rb") as handle:
            temp_dict12=pickle.load(handle)
            temp_dict12.pop(self.oop)
        with open(hw3,wb) as handle:
            pickle.dump(temp_dict12)
        with open(hw2,rb) as handle:
            ooop23 = pickle.load(handle) 
        with open(hw2,wb) as handle:
            ooop23.append(self.oop)
            pickle.dump(ooop23)
